...
//Asignar nombres a los valores de las teclas de flechas
#define ARRIBA 72
#define IZQUIERDA 75
#define DERECHA 77
#define ABAJO 80
...
//Funcion de ocultamiento de cursor
void OcultarCursor(){
	HANDLE hCon;
	hCon = GetStdHandle(STD_OUTPUT_HANDLE);
...
class NAVE{
	...
	public:
		...
		void COR(){corazones--;}
		void CORup(){if(corazones<5)corazones++;}  
		void pintar();
		void borrar();
		void mover();
...
//Colision del corazon
void HEAL::choque(class NAVE &N){
	if(x >= N.X() && x < N.X()+5 && y>= N.Y() && y <= N.Y()+2){
	N.CORup();
	N.borrar();
	N.pintar();
...
int main(){
	...
	list<HEAL*> C;
	list<HEAL*>::iterator itC;
	for(int i=0; i<1; i++){
		C.push_back(new HEAL(rand()%75 + 3, rand()%5 + 4));
	}		
